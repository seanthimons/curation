...
# This single command uses a glob pattern to read all matching parquet files
# from the 'final' directory into a single 'dsstox' table.
# This is more efficient and robust than iterating through files.
parquet_glob_path <- here("final", "dsstox_*.parquet")
DBI::dbExecute(
	dsstox_db,
	glue::glue(
		"CREATE OR REPLACE TABLE dsstox AS SELECT * FROM read_parquet('{parquet_glob_path}');"
	)
)
