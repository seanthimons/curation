...
#creates folders for the files----
{
  ...
  files_to_copy <- list.files(
    path = here('temp'),
    pattern = 'DisclosureList_[[:digit:]].csv$',
    full.names = TRUE
  )
  ...
  files_to_copy <- list.files(
    path = here('temp'),
    pattern = 'WaterSource_[[:digit:]].csv$',
    full.names = TRUE
  )
  ...
  files_to_copy <- list.files(
    path = here('temp'),
    pattern = 'FracFocusRegistry_\\d+.csv',
    full.names = TRUE
  )
...
file_list <- list.files(
  path = here('fracfocus', 'FracFocusCSV'),
  pattern = '^FracFocusRegistry_\\d+.csv',
  full.names = TRUE
)
...
mutate(
  ...
  rename(purpose = function_category, orig_purpose = description) %>%
  arrange(token)
