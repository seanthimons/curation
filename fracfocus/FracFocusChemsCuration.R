...
prepare_word <- function(word) {
  ...
  split_words <- stringr::str_split(string = word, pattern = '\\?', n = 2)[[1]]
  if (length(split_words) == 1) {
    ...
    if (nchar(split_words[[2]]) == 0) {
      ...
      temp_word <- paste0(
        urltools::url_encode(split_words[[1]]),
        '?',
...
chemical_equal_all = function(
  ...
  response <- httr::GET(
    url = paste0(Server, "/search/equal/", word),
    httr::add_headers(
      .headers = c(`Content-Type` = "application/json", `x-api-key` = API_key)
    )
  )
...
library(stringr)
newchemstocurate$chemical_name <- str_replace_all(
  newchemstocurate$chemical_name,
  "\\[\\[",
  ""
) #fixes two names that include double brackets (messes up API calls)
newchemstocurate$chemical_name <- str_replace_all(
  newchemstocurate$chemical_name,
  "\\]\\]",
  ""
...
uniquenames <- data.frame(unique(newchemstocurate$chemical_name[which(
  newchemstocurate$chemical_name != ""
)]))
...
curated_names <- curated_names[
  order(curated_names$cleanname, curated_names$rank),
]
...
suggested_names <- suggested_names[
  !str_detect(suggested_names$suggestions, "[[:upper:]]"),
] #remove INCHI key recs - these seem to be strange to me
...
suggested_names_curated <- suggested_names_curated[
  which(suggested_names_curated$acceptsuggestion == 1),
]
...
final_curated_suggestions <- final_curated_suggestions[
  order(final_curated_suggestions$suggname, final_curated_suggestions$rank),
]
final_curated_suggestions <- final_curated_suggestions[
  !duplicated(final_curated_suggestions$suggname),
]

#merge back in with "pre-suggestion" names
colnames(suggested_names_curated)[
  colnames(suggested_names_curated) == "suggestions"
] <- "suggname" #rename for merging
suggested_names_curated <- left_join(
  suggested_names_curated,
  final_curated_suggestions
)
...
uniqueCAS <- data.frame(unique(newchemstocurate$casrn[which(
  newchemstocurate$casrn != ""
)]))
...
#if there are two suggested DTSXIDs, take the one with highest curation rank
#order by cleanname then rank, remove any duplicates (this ensures best curation only retained)
final_curated_CAS <- final_curated_CAS[
  order(final_curated_CAS$cleancas, final_curated_CAS$rank),
]
final_curated_CAS <- final_curated_CAS[
  !duplicated(final_curated_CAS$cleancas),
]
...
colnames(curated_names)[
  colnames(curated_names) == "preferredName"
] <- "preferredName_by_name"
curated_names <- curated_names[, c(
  "clean_name",
  "suggested_name",
  "dtxsid_by_name",
  "preferredName_by_name"
...
colnames(suggested_names_curated)[
  colnames(suggested_names_curated) == "dtxsid"
] <- "dtxsid_by_name"
colnames(suggested_names_curated)[
  colnames(suggested_names_curated) == "preferredName"
] <- "preferredName_by_name"
suggested_names_curated <- suggested_names_curated[, c(
  "clean_name",
  "suggested_name",
  "dtxsid_by_name",
  "preferredName_by_name"
...
all_curated_chemicals_by_name <- bind_rows(
  curated_names,
  suggested_names_curated
)

##Rename the rows for the "by CAS" curations
colnames(final_curated_CAS)[
  colnames(final_curated_CAS) == "cleancas"
] <- "clean_casrn" #just clean up labeling for consistency
colnames(final_curated_CAS)[
  colnames(final_curated_CAS) == "dtxsid"
] <- "dtxsid_by_casrn"
colnames(final_curated_CAS)[
  colnames(final_curated_CAS) == "preferredName"
] <- "preferredName_by_casrn"
final_curated_CAS <- final_curated_CAS[, c(
  "clean_casrn",
  "dtxsid_by_casrn",
  "preferredName_by_casrn"
)]
...
colnames(newchemstocurate)[
  colnames(newchemstocurate) == "chemical_name"
] <- "clean_name"
colnames(newchemstocurate)[
  colnames(newchemstocurate) == "casrn"
] <- "clean_casrn"

#merge the curations by name
cleaned_curated_data <- left_join(
  newchemstocurate,
  all_curated_chemicals_by_name
)
...
cleaned_curated_data$conflict[which(
  !j &
    !k &
    cleaned_curated_data$dtxsid_by_name != cleaned_curated_data$dtxsid_by_casrn
)] <- 1
