...
if (load == 1) {
  setwd(
    "C:/Users/kisaacs1/OneDrive - Environmental Protection Agency (EPA)/ProducedWater_CESER/FracFocusCSV/"
  )
  ...
  ingredients1 <- file1[
    which(file1$IngredientsId != ""),
    c(
      "WellName",
      "IngredientsId",
      "IngredientCommonName",
      "CASNumber",
      "IngredientName",
      "Purpose",
      "PurposeId",
  ...
  ingredients10 <- file10[
    which(file10$IngredientsId != ""),
    c(
      "WellName",
      "IngredientsId",
      "IngredientCommonName",
  ...
  ingredients11 <- file11[
    which(file11$IngredientsId != ""),
    c(
      "WellName",
      "IngredientsId",
      "IngredientCommonName",
  ...
  ingredients12 <- file12[
    which(file12$IngredientsId != ""),
    c(
      "WellName",
      "IngredientsId",
      "IngredientCommonName",
  ...
  ingredients13 <- file13[
    which(file13$IngredientsId != ""),
    c(
      "WellName",
      "IngredientsId",
      "IngredientCommonName",
  ...
  setwd(
    "C:/Users/kisaacs1/OneDrive - Environmental Protection Agency (EPA)/ProducedWater_CESER/Output/"
  )
  ...
  uniqueingredientspurposepairs <- unique(ingredients[, c(
    "IngredientName",
    "CASNumber",
    "Purpose"
  )])
...
library("dplyr")
setwd(
  "C:/Users/kisaacs1/OneDrive - Environmental Protection Agency (EPA)/ProducedWater_CESER/"
)
...
uniquepurposes2 <- uniquepurposes %>%
  mutate(
    individualpurposes = strsplit(as.character(Reported.Functional.Use), ",")
  ) %>%
  unnest(individualpurposes)

uniquepurposes2$Reported.Functional.Use <- uniquepurposes2$individualpurposes
uniquepurposes2 <- uniquepurposes2[,
  !colnames(uniquepurposes2) == "individualpurposes"
]
uniquepurposes_indexed <- uniquepurposes2 #maps back to reported purposes

#unique individual purposes

uniqueindividualpurposes <- data.frame(unique(
  uniquepurposes_indexed$Reported.Functional.Use
))
...
uniquefunctioncurations <- unique(factotumfunction[, c(
  "Harmonized.Functional.Use",
  "Reported.Functional.Use"
)])
#remove incorrect mappings in facotum
uniquefunctioncurations <- uniquefunctioncurations[
  which(uniquefunctioncurations$Reported.Functional.Use != "Friction Reducer"),
]
...
uniquefunctioncurations$Reported.Functional.Use <- trimws(tolower(
  uniquefunctioncurations$Reported.Functional.Use
))
uniquefunctioncurations <- uniquefunctioncurations[
  which(uniquefunctioncurations$Harmonized.Functional.Use != ""),
]
uniqueindividualpurposes$Reported.Functional.Use <- trimws(tolower(
  uniqueindividualpurposes$IndividualPurpose
))


uniquestandardizedpurposes <- data.frame(unique(
  uniqueindividualpurposes$Reported.Functional.Use
))
colnames(uniquestandardizedpurposes) <- "Reported.Functional.Use"

#merge
curatedpurposes <- left_join(
  uniquestandardizedpurposes,
  uniquefunctioncurations
)

tocurate <- curatedpurposes[
  which(is.na(curatedpurposes$Harmonized.Functional.Use)),
]
...
tocurate$Harmonized.Functional.Use[grepl(
  "gel",
  tocurate$Reported.Functional.Use
)] <- "Viscosity modifier"
tocurate$Harmonized.Functional.Use[grepl(
  "guar",
  tocurate$Reported.Functional.Use
...
tocurate$Harmonized.Functional.Use[grepl(
  "anionic polymer in emulsion form",
  tocurate$Reported.Functional.Use
)] <- "Viscosity modifier"
tocurate$Harmonized.Functional.Use[grepl(
  "flocc",
...
tocurate$Harmonized.Functional.Use[grepl(
  "emulsion reducer/increases viscosity",
  tocurate$Reported.Functional.Use
)] <- "Viscosity modifier"
...
tocurate$Harmonized.Functional.Use[
  grepl("transport", tocurate$Reported.Functional.Use) &
    grepl("additive", tocurate$Reported.Functional.Use)
] <- "Viscosity modifier" #per research
...
tocurate$Harmonized.Functional.Use[grepl(
  "amphoteric foamer for water/acid",
  tocurate$Reported.Functional.Use
)] <- "Foamant"
tocurate$Harmonized.Functional.Use[
  grepl("foam", tocurate$Reported.Functional.Use) &
    grepl("anti", tocurate$Reported.Functional.Use)
] <- "Defoamer"
...
tocurate$Harmonized.Functional.Use[grepl(
  "kcl",
  tocurate$Reported.Functional.Use
)] <- "Stabilizing agent" #assume kcl etc is stabilizer per research
tocurate$Harmonized.Functional.Use[grepl(
  "oilfield shale control",
  tocurate$Reported.Functional.Use
)] <- "Stabilizing agent" # per research
tocurate$Harmonized.Functional.Use[grepl(
  "shale inhibitor",
  tocurate$Reported.Functional.Use
)] <- "Stabilizing agent" # per research
...
tocurate$Harmonized.Functional.Use[grepl(
  "hcl",
  tocurate$Reported.Functional.Use
)] <- "pH regulating agent" #assume hcl is pH regulating agent per research
...
tocurate$Harmonized.Functional.Use[
  grepl("salt", tocurate$Reported.Functional.Use) &
    grepl("inh", tocurate$Reported.Functional.Use)
] <- "Anti-scaling agent"
tocurate$Harmonized.Functional.Use[
  grepl("sale", tocurate$Reported.Functional.Use) &
    grepl("inh", tocurate$Reported.Functional.Use)
] <- "Anti-scaling agent"
tocurate$Harmonized.Functional.Use[grepl(
  "sclae",
...
tocurate$Harmonized.Functional.Use[grepl(
  "sufactant",
  tocurate$Reported.Functional.Use
)] <- "Surfactant (surface active agent)" #per research
tocurate$Harmonized.Functional.Use[grepl(
  "wettability",
  tocurate$Reported.Functional.Use
)] <- "Surfactant (surface active agent)" #per research
...
tocurate$Harmonized.Functional.Use[grepl(
  ...
  tocurate$Reported.Functional.Use
)] <- "Silt suspending agent (HF)" #per research
tocurate$Harmonized.Functional.Use[grepl(
  "silt fines suspender",
  tocurate$Reported.Functional.Use
)] <- "Silt suspending agent (HF)" #per research
tocurate$Harmonized.Functional.Use[grepl(
  "silt suspender",
...
tocurate$Harmonized.Functional.Use[
  grepl("non", tocurate$Reported.Functional.Use) &
    grepl("emul", tocurate$Reported.Functional.Use)
] <- "Demulsifier"
tocurate$Harmonized.Functional.Use[
  grepl("non", tocurate$Reported.Functional.Use) &
    grepl("emus", tocurate$Reported.Functional.Use)
] <- "Demulsifier"
tocurate$Harmonized.Functional.Use[grepl(
  "de-emulsifier",
  tocurate$Reported.Functional.Use
...
tocurate$Harmonized.Functional.Use[grepl(
  "antimicobial solution",
  tocurate$Reported.Functional.Use
)] <- "Biocide"
tocurate$Harmonized.Functional.Use[grepl(
  "antimicrovial solution",
  tocurate$Reported.Functional.Use
)] <- "Biocide"
...
tocurate$Harmonized.Functional.Use[
  grepl("o2", tocurate$Reported.Functional.Use) &
    grepl("scav", tocurate$Reported.Functional.Use)
] <- "O2 scavenger (HF)"
tocurate$Harmonized.Functional.Use[
  grepl("ox", tocurate$Reported.Functional.Use) &
    grepl("scav", tocurate$Reported.Functional.Use)
] <- "O2 scavenger (HF)"
tocurate$Harmonized.Functional.Use[grepl(
  "oxygen savenger",
...
tocurate$Harmonized.Functional.Use[
  grepl("h2", tocurate$Reported.Functional.Use) &
    grepl("scav", tocurate$Reported.Functional.Use)
] <- "H2S scavenger (HF)"
tocurate$Harmonized.Functional.Use[
  grepl("sulf", tocurate$Reported.Functional.Use) &
    grepl("scav", tocurate$Reported.Functional.Use)
] <- "H2S scavenger (HF)"
tocurate$Harmonized.Functional.Use[grepl(
  "h2s inhibitor",
...
k <- match(
  curatedpurposes$Reported.Functional.Use,
  tocurate$Reported.Functional.Use
)
curatedpurposes$Harmonized.Functional.Use_new <- tocurate$Harmonized.Functional.Use[
  k
]
j <- which(is.na(curatedpurposes$Harmonized.Functional.Use))
curatedpurposes$Harmonized.Functional.Use[
  j
] <- curatedpurposes$Harmonized.Functional.Use_new[j]

curatedpurposes <- curatedpurposes[, c(
  "Reported.Functional.Use",
  "Harmonized.Functional.Use"
)]

#fix a few curations that may need updating from CPDat curations, or that weren't handled by the above logic
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "polymer"
)] <- "Viscosity modifier"
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "resin"
)] <- NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "other"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "miscellaneous"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "additive"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "various"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "processing aid"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "oil"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "additives"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "other additive"
)] <- NA # we just leave these NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "unknown"
)] <- NA # we just leave these NA


curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "bleach"
)] <- "Biocide"
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "bleaching agent"
)] <- "Biocide"

curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use == "paraffin"
)] <- "Paraffin inhibitor (HF)"
...
new <- data.frame(
  c("scale inhibito & nitrate reducing bacteria", "Anti-scaling agent"),
  ...
  c("non-emulsifier/ surfactant", "Surfactant (surface active agent)"),
  c(
    "surfactant / non-emulsifier / flow back",
    "Surfactant (surface active agent)"
  ),
  ...
  c(
    "borate crosslinker - stabilizer - ph buffer",
    "Chemical reaction regulator"
  )
)
...
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use ==
    "propping agent. acid corrosion inhibitor. friction reducer. biocide. microbiocide. microbiocide. blocking agent. clean perforations"
)] <- NA
curatedpurposes$Harmonized.Functional.Use[which(
  curatedpurposes$Reported.Functional.Use ==
    "paraffin/ne/scale inh./bio/h2s scav."
)] <- NA
