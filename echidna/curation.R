    ...
    possibly(
      ~ {
        ...
        logged_in %>%
          ...
          xml2::write_html(
            .,
            file = paste0(str_remove(.x, pattern = 'view.php\\?cid='), '.html')
          )
        #Sys.sleep(5)
      }
    ),
          ...
          possibly(
            ~ {
              tbl <- page %>%
                html_elements(
                  .,
                  xpath = paste0('/html/body/div[1]/div/table[', .y, ']')
                ) %>%
                        ...
                        ~ {
                          ...
                          map2(
                            ...
                            ~ {
                              ...
                              tibble(
                                column = .y,
                                title = if (length(spans) > 0) {
                                  html_attr(spans, "title")
                                } else {
                                  NA
                                }
                              )
...

        {
          d1 <- dat %>%
            ...
            filter(
              str_detect(
                dat,
                pattern = 'Listed|Detected|Reported|Requested|3D|Note',
                negate = TRUE
              )
            )
            ...
            keep(~ all(c("HQ?") %in% names(.x))) %>%
            {
              if (length(.) == 0) {
                ...
                list_rbind(.) %>%
                  pivot_longer(
                    .,
                    cols = any_of(c('Water type', 'Endpoint')),
                    names_to = 'Endpoint',
                    values_to = 'dat',
                    values_drop_na = T,
                    values_transform = as.character
                  ...
                  pivot_longer(
                    .,
                    cols = any_of(c('Concentration', 'Dose')),
                    values_to = 'val',
                    values_drop_na = T,
                    values_transform = as.character
                  )
