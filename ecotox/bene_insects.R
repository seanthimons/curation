...
tbl(eco_con, 'species') %>% 
  ...
  filter(
    str_detect(family, pattern = 'Coccinellidae|Cecidomyiidae|Reduviidae|Nabidae|Chrysopidae|Anthocoridae|Lampyridae|Cantharidae|Syrphidae|Tachinidae|Trichogrammatidae|Braconidae|Ichneumonidae|Mantidae|Megachilidae|Apidae')
  )
...
tbl(eco_con, 'species') %>% 
  ...
  filter(
    str_detect(eco_group, pattern = 'Fungi|Flowers|Molluscs|Amphibians|Reptiles|Moss, Hornworts')
  ) %>% 
  pull(species_number)
