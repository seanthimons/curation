...
download.file(paste0('https://gaftp.epa.gov/ecotox/', resp$file[which.max(resp$date)]), destfile = here('ecotox', 'ecotox.zip'))
...
walk(
  list.files(pattern = ".txt")[!str_detect(list.files(pattern = ".txt"), pattern = "release")],
  function(x) {
