...
raw_pol_cas_src <- ct_search(
  query = raw_pol_cas$cas,
  request_method = 'GET',
  search_method = 'equal'
) %>%
  select(raw_search, dtxsid)
...
pol_final <- bind_rows(
  ...
  pivot_longer(
    .,
    cols = c(dtxsid, internalTrackingNumber),
    values_to = 'v',
    values_drop_na = TRUE
  ) %>%
  select(-name) %>%
  ...
  result_idx_char_cur$A %<>%
    mutate(
      result = result %>%
        ...
        str_replace_all(., pattern = '\\+', replacement = 'E') %>%
        str_squish(),
