		...
		walk(
			...
			~ {
				table_name <- basename(.x) %>%
					str_extract(., pattern = 'dsstox')
				# str_remove("\\.parquet$") %>%
				# str_extract(., pattern = '^dsstox_.*') # Extract full table name like "dsstox_chemicals"

				# Instead of reading into R memory, directly create a table from the parquet file
				dbExecute(
					dsstox_db,
					paste0("CREATE TABLE ", table_name, " AS SELECT * FROM '", .x, "'")
				)
			}
		)
...
pt_smiles <- pt$elements %>%
	select(Symbol)

pt_dsstox <- tbl(dsstox_db, 'dsstox') %>%
	select(DTXSID, MOLECULAR_FORMULA, SMILES, CASRN) %>%
	collect() %>%
	inner_join(., pt_smiles, join_by(MOLECULAR_FORMULA == Symbol)) %>%
	filter(!str_detect(SMILES, pattern = "\\+|\\-"))
...
pt$elements <-
	left_join(
		pt$elements,
		pt_dsstox,
		join_by(Symbol == MOLECULAR_FORMULA)
	) %>%
...
nuclides <- bow('https://en.wikipedia.org/wiki/Table_of_nuclides') %>%
	...
	left_join(
		...
		tbl(dsstox_db, 'dsstox') %>%
			select(DTXSID, CASRN, SMILES) %>%
			filter(str_detect(SMILES, isotope_regex_pattern)) %>%
			collect(),
		join_by(smiles == SMILES)
	)
