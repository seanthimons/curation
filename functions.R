...
similar_hash <- function(target, target_by, dict, dict_by, osa = NA, lcs = NA, hamm = NA, cos = NA, jw = NA, jacc = NA){
  
  target_by <- enquo(target_by)
  dict_by <- enquo(dict_by)
  ...
  init <-
    map(
      methods,
      ~ {
        stringdist_join(x = target, y = dict,
                        by = c(quo_name(target_by) = quo_name(dict_by)),
                        # mode = 'right',
                        method = .x,
                        # max_dist = 9,,
                        ignore_case = TRUE,
...
similar_hash <- function(target, target_by, dict, dict_by, osa = 1, lcs = 1, hamm = 1, cos = NA, jw = NA, jacc = NA){
...

{
  ...
  hash_4 <-
    ...
    list_rbind(names_to = "method") %>%
    filter(
      method == 'cosine' & dist <= quantile(dist, 0.05) | method == 'jw' & dist <= quantile(dist, 0.05)
    )
