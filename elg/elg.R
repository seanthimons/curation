...
pollutant_categories <- map(
  dat$pollutant_categories$id,
  ~ {
    resp <- GET(
      url = paste0('https://owapps.epa.gov/elg/api/pollutantCategory/?id=', .x)
    )
    .x <- content(resp)
  },
  .progress = T
...
poll_limits <- map2(
  poll_temp$pollutantId,
  poll_temp$pointSourceCategoryCode,
  function(id, code) {
    resp <- GET(
      url = paste0(
        'https://owapps.epa.gov/elg/api/pollutantLimitations?pollutantId=',
        id,
...
poll_lta <- map(
  poll_temp$limitationId,
  function(id) {
    resp <- GET(
      url = paste0('https://owapps.epa.gov/elg/api/limitation?id=', id)
    )
    df <- content(resp)
    return(df)
  },
...
walk(
  ...
  function(id) {
    ...
    download.file(
      url = paste0(
        'https://owapps.epa.gov/elg/api/multiCriteriaSearch?pointSourceCategoryCode=',
        id,
        '&sicCode=&naicsCode=&pollutantId=&pollutantGroupId=&rangeLow=&rangeHigh=&rangeUnitCode=&treatmentTechnologyCode=&treatmentTechnologyGroup=&filterPointSourceCategoryCode=&filterPollutantId=&filterTreatmentId=&download=true'
      ),
