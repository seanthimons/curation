...
raw_eraf <- rio::import(file = here("eraf","SurfaceWaterBenchmarks_ERAF.WG18.xlsx"))

names(raw_eraf) <- 
  c('chemical', 'cas', 'fw_chron', 'fw_acute', 'fw_source', 'sw_chron', 'sw_acute', 'sw_source')
...
eraf_cur <- eraf %>% 
  mutate(., 
         ...
         ft = str_remove_all(chemical, pattern = '[[:alnum:]]|\\(|\\)|\\.|\\,|\\/|\\-|\\s'),
         ft = na_if(ft, ""),
  ...
  left_join(., cit, join_by(source_fix == key), relationship = "many-to-many") %>% 
  rename(meta = msg.x, cit = msg.y) %>% 
  mutate(
    cit = case_when(
      source_fix == 'w' ~ 'Minimum freshwater acute value in ECOTOX divided by default ACR',
      source_fix == 'y' ~ 'Maine State Standard',
      .default = cit
    ),
    
    origin_category = case_when(
      str_detect(cit, pattern = 'EPA|National|GLI|OPP|ECOSAR|ECOTOX') ~ 'Federal',
...
eraf_dis <- ComptoxR::ct_search(type = 'string', search_param = 'equal', query = eraf_dis$cas_fix)
