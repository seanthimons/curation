...
tbls <- tbls_raw %>%
  ...
  filter(!str_detect(name, pattern = 'Number of countries'), !str_detect(value, pattern = 'None|No value')) %>% # Filter out rows where the name contains 'Number of countries' or the value contains 'None' or 'No value'
  mutate(orig_value = value) %>% # Create a new column called 'orig_value' and assign it the value of the 'value' column
  separate(value, into = c("value1", "value2", "value3"), sep = "\\s{1,2}",  extra = "merge") %>% # Split the 'value' column into three columns called 'value1', 'value2', and 'value3' using one or two whitespace characters as the separator
  ...
  mutate(
    ...
    num_value = case_when(
      str_detect(compound, pattern = 'pH') & is.na(num_value) ~ as.numeric(unit),
    ...
    unit = case_when(
      is.na(unit) & compound %ni% c('Trihalomethanes (Total)', 'Temperature', 'Taste', 'Escherichia coli (Faecal coliforms, Thermotolerant coliforms)') ~ 'mg/l',
      is.na(unit) & compound == 'Trihalomethanes (Total)' ~ 'TTHM',
      is.na(unit) & compound == 'Temperature' ~ 'C',
      is.na(unit) & compound == 'Taste' ~ 'DN',
      is.na(unit) & compound == 'Escherichia coli (Faecal coliforms, Thermotolerant coliforms)' ~ 'CFU/100 ml',
      compound == 'Escherichia coli (Faecal coliforms, Thermotolerant coliforms)' ~ 'CFU/100 ml',
      str_detect(compound, pattern = 'pH') ~ 'standard unit',
      unit == 'cfu' ~ 'CFU/ ml',
      unit == 'per' ~ 'CFU/100 ml',
      compound ==  'Clostridium perfringens (Sulphite-reducing anaerobes)' & name == 'Maximum value set' & unit == 'per' ~ 'CFU/20 ml',
